...
export default function WorkLifeNew({ changeStep, handleComplete }) {
  ...
  const handleSaveButton = async () => {
    try {
      const data = workData.map((item, index) => {
        if (index === 0) {
          return { ...item, start_date: "2023-09-15", end_date: "2023-09-15" };
        } else {
          return item;
        }
      });
      const { payload } = await dispatch(addWorkData(noWorkExp ? [] : data));
  ...
  const getCVWorkLifeData = async () => {
    ...
    if (payload?.status == "success") {
      ...
      payload?.data.length === 0 && setWorkData([...workData, WORK]);
    } else if (payload?.status == "error") {
      dispatch(
        setAlert({
          show: true,
