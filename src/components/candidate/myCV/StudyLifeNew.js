...
export default function StudyLifeNew({ changeStep, handleComplete }) {
  ...
  const getCVStydyLifeData = async () => {
    ...
    if (payload?.status == "success") {
      ...
      payload?.data?.qualification?.length === 0 &&
        setStudyData((prevStudyData) => ({
          ...prevStudyData,
          qualification: [...prevStudyData.qualification, QUALIFICATION],
        }));
    } else if (payload?.status == "error") {
      dispatch(
        setAlert({
