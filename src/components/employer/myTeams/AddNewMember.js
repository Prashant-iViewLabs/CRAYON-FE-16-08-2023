...
const AddNewMember = () => {
  ...
  const getAllData = async () => {
    try {
      ...
      setMemberData({
        ...MEMBER_INFO,
        company_id: company.payload.data.find(
  ...
  const handleCompVal = (event, newValue, id) => {
    ...
    let defaultCompID = companies?.find(
      (title) => title.name === companyName
    ).company_id;

    newmemberData = {
      ...memberData,
      [id]: newValue?.company_id || newValue?.inputValue || defaultCompID,
    };
      ...
      >
        ...
        <Grid item xs={12} md={6}>
          ...
          <AutoComplete
            ...
            value={
              companies?.find(
                (title) => title.company_id === memberData?.company_id
              ) ||
              companyName ||
